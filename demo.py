...
snapshots = gmatch.util.get_snapshots(mesh)

# visualize the snapshots
gmatch.util.vis_snapshots(snapshots)
...
poses_src = [gmatch.util.mat2pose(M_ex) for M_ex in M_ex_list]
...
cld_dst = gmatch.util.depth2cld(depth_dst * 0.0001, cam_intrin)
...
""" GMatch-SIFT """
# put data into gmatch.util.MatchData
...
# gmatch.util.plot_matches(match_data.imgs_src[i], match_data.img_dst, uv1_m, uv2_m)
...
for i in range(len(clds_src)):
    pts = gmatch.util.transform(clds_src[i][masks_src[i] != 0], poses_src[i])
